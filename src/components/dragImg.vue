...
export default {
    ...
    methods: {
        ...
        checkimgLoaded() {
            ...
            const tipHeight = 20;
            const x = halfWidth + Math.ceil(Math.random() * (halfWidth - barWidth - this.barRadius - 5));
            let y =
                refreshHeigth +
                Math.floor(Math.random() * (imgHeight - barWidth - refreshHeigth - tipHeight));
        ...
        dragMoving(e) {
            if (this.isMoving && !this.isPassing) {
                const x = (e.pageX || e.touches[0].pageX) - this.x;
                const handler = this.$refs.handler;
                handler.style.left = `${x}px`;
                this.$refs.progressBar.style.width = `${x + this.height / 2}px`;
                this.$refs.movecanvas.style.left = `${x}px`;
            }
        },
        dragFinish(e) {
            if (this.isMoving && !this.isPassing) {
                const x = (e.pageX || e.changedTouches[0].pageX) - this.x;
                if (Math.abs(x - this.clipBarx) > this.diffWidth) {
                    this.isOk = true;
                    const that = this;
                    setTimeout(() => {
